...
## keyboard callback
	...
	modes = fcuModes()
		...
		self.roll_d = numpy.deg2rad(10.0)
		self.pitch_d = 0.0
		self.yaw_d = 0.0#numpy.deg2rad(180.0)
		q = quaternion_from_euler(self.roll_d, self.pitch_d, self.yaw_d)
        	self.at_des.pose.orientation = Quaternion(*q)
		...
		#self.at.orientation = self.at_des.pose.orientation
        	#self.at.thrust = 0.1
	self.time+=self.dt
	print(self.time,self.roll,self.roll_d)		
...
def main():
    global archivo
    # initiate node
    rospy.init_node('experiment_sys_theta', anonymous=True)
