...
## keyboard callback
	...
	self.file_csv=open('/home/alexmorfin/catkin_ws/src/python_ob_docto/scripts/prueba_%s.txt'%(archivo),'a')
	...
	modes = fcuModes()
		...
		self.roll_d = 0.0
		self.pitch_d = 0.0
		self.yaw_d = numpy.deg2rad(15.0)#numpy.deg2rad(180.0)
		q = quaternion_from_euler(self.roll_d, self.pitch_d, self.yaw_d)
        	self.at_des.pose.orientation = Quaternion(*q)
		#print(self.at_des.pose.orientation)
		self.at.orientation = self.at_des.pose.orientation
		...
		self.at.orientation = self.at_des.pose.orientation
        	self.at.thrust = 0.6
	elif self.time >15.0 and self.time <=20.0:
		self.roll_d = 0.0
		self.pitch_d = 0.0
		self.yaw_d = numpy.deg2rad(-30.0)#numpy.deg2rad(180.0)
		q = quaternion_from_euler(self.roll_d, self.pitch_d, self.yaw_d)
        	self.at_des.pose.orientation = Quaternion(*q)
		#print(self.at_des.pose.orientation)
		self.at.orientation = self.at_des.pose.orientation
        	self.at.thrust = 0.6
	elif self.time >20.0 and self.time <=25.0:
		self.roll_d = 0.0
		self.pitch_d = 0.0
		self.yaw_d = 0.0#numpy.deg2rad(180.0)
		q = quaternion_from_euler(self.roll_d, self.pitch_d, self.yaw_d)
		...
		self.at.orientation = self.at_des.pose.orientation
        	self.at.thrust = 0.6
	elif self.time >25.0 and self.time <=30.0:
		self.roll_d = 0.0
		self.pitch_d = 0.0
		self.yaw_d = numpy.deg2rad(90.0)#numpy.deg2rad(180.0)
		q = quaternion_from_euler(self.roll_d, self.pitch_d, self.yaw_d)
        	self.at_des.pose.orientation = Quaternion(*q)
		#print(self.at_des.pose.orientation)
		self.at.orientation = self.at_des.pose.orientation
		...
		#self.at.orientation = self.at_des.pose.orientation
        	#self.at.thrust = 0.1
	self.time+=self.dt
	print(self.time,self.yaw,self.yaw_d)		
...
def main():
    global archivo
    # initiate node
    rospy.init_node('experiment_sys_psi', anonymous=True)
    ...
    last=open('/home/alexmorfin/catkin_ws/src/python_ob_docto/scripts/last.txt','r')
    archivo=last.read()
    archivo=int(archivo)+1
    last=open('/home/alexmorfin/catkin_ws/src/python_ob_docto/scripts/last.txt','w')
    last.write(str(archivo))
    last.close()
	
    #Crear Archivo de datos o de prueba
    f_csv=open('/home/alexmorfin/catkin_ws/src/python_ob_docto/scripts/prueba_%s.txt'%(archivo),'a')
