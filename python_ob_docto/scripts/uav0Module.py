...
## Data ##
	...
	self.file_csv=open('/home/alexmorfin/catkin_ws/src/python_ob_docto/scripts/Pruebas_multi_sys/prueba_%s_uav0.txt'%(self.archivo),'a')
	self.datos=str([t, uz, alt, vel_alt, uthe, roll, uphi, pitch, upsi, yaw])
	self.file_csv.write(self.datos+'\n')
	self.file_csv.close()
## Update setpoint message
    def updateSp(self,ref_z,kp,kd):
       	alt=self.pos_uav.z
	vel_alt=self.vel_uav.z
	self.data()
	modes = fcuModes()
	...
	ref_vel_z=0.0
	u0z=(-kp*(alt-ref_z)-kd*(vel_alt-ref_vel_z)+self.a2*alt+self.a1*vel_alt)
        self.at.thrust = u0z
	#else:
