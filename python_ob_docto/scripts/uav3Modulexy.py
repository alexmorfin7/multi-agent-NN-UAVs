...
class fcuModes:
    ...
    def setTakeoff(self):
    	rospy.wait_for_service('uav3/mavros/cmd/takeoff')
    	try:
    		takeoffService = rospy.ServiceProxy('uav3/mavros/cmd/takeoff', mavros_msgs.srv.CommandTOL)
    ...
    def setArm(self):
        ...
        try:
            armService = rospy.ServiceProxy('uav3/mavros/cmd/arming', mavros_msgs.srv.CommandBool)
            armService(True)
        except rospy.ServiceException, e:
    ...
    def setStabilizedMode(self):
        ...
        try:
            flightModeService = rospy.ServiceProxy('uav3/mavros/set_mode', mavros_msgs.srv.SetMode)
...
class Controller:
    ...
    def __init__(self, archivo = 0,x_init=0,y_init=0):
        # Drone state
        self.state = State()
	...
	self.brfz= np.zeros((9,1))
	self.brfzf= np.zeros((9,1))
	self.sigmazf=np.zeros((2,1))
	self.deltazf=np.zeros((2,1))
	self.Pz=np.zeros((9,9))
	self.Qz=np.zeros((9,1))
	self.wz=np.zeros((9,1))
	self.brfx= np.zeros((9,1))
	self.brfxf= np.zeros((9,1))
	self.sigmaxf=np.zeros((2,1))
	self.deltaxf=np.zeros((2,1))
	self.Px=np.zeros((9,9))
	self.Qx=np.zeros((9,1))
	self.wx=np.zeros((9,1))
	self.brfy= np.zeros((9,1))
	self.brfyf= np.zeros((9,1))
	self.sigmayf=np.zeros((2,1))
	self.deltayf=np.zeros((2,1))
	self.Py=np.zeros((9,9))
	self.Qy=np.zeros((9,1))
	self.wy=np.zeros((9,1))
...
## Data ##
	...
	self.file_csv=open('/home/alexmorfin/catkin_ws/src/python_ob_docto/scripts/Pruebas_multi_sys_xy/prueba_%s_uav3.txt'%(self.archivo),'a')
...
## Update setpoint message
	...
	self.pitch_d = atan((float(ux)*cos(self.yaw_d)+float(uy)*sin(self.yaw_d))/u)
	self.roll_d = atan(-(float(ux)*sin(self.yaw_d)+float(uy)*cos(self.yaw_d))*cos(self.pitch_d)/u)#0.0
	...
	self.at.orientation = self.at_des.pose.orientation
        self.at.thrust = float(u)/self.a3+0.51#+self.a2*alt+self.a1*vel_alt
	self.data(ux,uy)
...
############################### Neural Network ##################################
	...
	b=0.8
	inp_x=np.array([[self.pos_uav.x],[(self.vel_uav.x)*1.5]])
	inp_y=np.array([[self.pos_uav.y],[(self.vel_uav.y)*1.5]])
	#print(inp_b)
    	#pdb.set_trace()
    	for j in range(nn):
...
####### Matrix Q #############
	...
	#print(psi)
    	#pdb.set_trace()
	for r in range(nn):
                self.Qx[r]+= T*(-lam*self.Qx[r]+ self.brfxf[r]*(psix[0]))
		self.Qy[r]+= T*(-lam*self.Qy[r]+ self.brfyf[r]*(psiy[0]))
	#print(self.Q1)
    	#pdb.set_trace()
####### Vector H_brf #########
...
############ uPDATE LAW ####
	...
	#print(self.w1)
    	#pdb.set_trace()         
        Fx = np.dot(self.wx.transpose(),self.brfx)*0.001
	Fy = np.dot(self.wy.transpose(),self.brfy)*0.001
	#print(F1)
    	#pdb.set_trace()
	#print(F1)
	return Fx, Fy
