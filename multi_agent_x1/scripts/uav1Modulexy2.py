...
## Data ##
	...
	self.file_csv=open('/home/alexmorfin/catkin_ws/src/multi_agent_x1/scripts/Pruebas_multi_sys_xy_2/prueba_%s_uav1.txt'%(self.archivo),'a')
...
## Update setpoint message
	...
	self.at.orientation = self.at_des.pose.orientation
        self.at.thrust = float(u)/self.a3+0.56#+self.a2*alt+self.a1*vel_alt
	self.data(ux,uy)
...
##Red Neuronal
	...
	b=0.9
	inp_b=np.array([[self.pos_uav.z],[(self.vel_uav.z)*0.5]])
	#print(inp_b)
    	#pdb.set_trace()
    	for j in range(nn):
...
############################### Neural Network ##################################
	...
	#c=np.array([[-4.5 ,-4.5 ,-4.5 ,-4.5 ,-4.5 ,-4.5 ,-4.5, -3. ,-3. ,-3. ,-3. ,-3. ,-3. ,-3. ,-1.5 ,-1.5 ,-1.5 ,-1.5 ,-1.5 ,-1.5 ,-1.5 ,0. ,0. ,0. ,0. ,0. ,0. ,0. ,1.5 ,1.5 ,1.5 ,1.5 ,1.5 ,1.5 ,1.5 ,3. ,3. ,3. ,3. ,3. ,3. ,3. ,4.5 ,4.5 ,4.5 ,4.5 ,4.5 ,4.5 ,4.5], [-4.5 ,-3. ,-1.5 ,0. ,1.5 ,3. ,4.5 ,-4.5 ,-3.0 ,-1.5 ,0. ,1.5 ,3. ,4.5 ,-4.5 ,-3. ,-1.5 ,0. ,1.5 ,3. ,4.5,-4.5 ,-3. ,-1.5 ,0. ,1.5 ,3. ,4.5 ,-4.5 ,-3. ,-1.5 ,0. ,1.5 ,3. ,4.5,-4.5 ,-3. ,-1.5 ,0. ,1.5 ,3. ,4.5,-4.5 ,-3. ,-1.5 ,0. ,1.5 ,3. ,4.5]])
	b=9.
	inp_x=np.array([[self.pos_uav.x*.01],[(self.vel_uav.x)*0.001]])
	inp_y=np.array([[self.pos_uav.y*.01],[(self.vel_uav.y)*0.001]])
	#print(inp_b)
    	#pdb.set_trace()
    	for j in range(nn):
...
def main():
    ...
    last=open('/home/alexmorfin/catkin_ws/src/multi_agent_x1/scripts/Pruebas_multi_sys_2/last.txt','r')
    archivo=last.read()
    archivo=int(archivo)+1
    last=open('/home/alexmorfin/catkin_ws/src/multi_agent_x1/scripts/Pruebas_multi_sys_2/last.txt','w')
    last.write(str(archivo))
    last.close()
	
    #Crear Archivo de datos o de prueba
    f_csv=open('/home/alexmorfin/catkin_ws/src/multi_agent_x1/scripts/Pruebas_multi_sys_2/prueba_%s.txt'%(archivo),'a')
